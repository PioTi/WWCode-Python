...
while is_done != 'y':
    ...
    if item in products.keys():
        ...
        if item in purchase.keys():
            count = purchase[item]+1
            purchase[item] = count
        else :
            count +=1
            purchase[item] = count
    else:
         print('Item selected is void ! '+item) 
...
for nameItem,countItem in purchase.items():
    if nameItem in products:
       # totalAmount = products[nameItem] * float(countItem) 
        print(nameItem  +' {:.2f}'.format(products[nameItem])+' * {} = '.format(countItem) + '{:.2f}'.format(products[nameItem] * float(countItem)))
